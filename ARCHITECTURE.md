...
| 같은 제약, 다른 엔드포인트 | 빈 `language` 는 확장자를 `.txt` 로 만들어 rereview 가 **영구 거부**하는 파일을 저장소에 남긴다. 세 엔드포인트가 같은 하류 제약을 공유한다 | 규칙을 `require_language()` 한 곳에 두고 셋이 호출한다. `require_platform()` 도 같은 이유로 한 곳에 둔다 |
