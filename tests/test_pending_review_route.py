...
def test_requires_language(client):
    r = client.post("/api/review/pending", json={**_BODY, "language": ""})
    assert r.status_code == 400
    assert "언어" in r.json()["detail"]


def test_requires_code(client):
    r = client.post("/api/review/pending", json={**_BODY, "code": "   "})
    assert r.status_code == 400


def test_saves_pending_row_after_successful_push(client, monkeypatch):
    monkeypatch.setattr(pending_review, "push_review_bundle",
                        lambda *a, **k: "백준/Bronze/1000번. A+B")

    body = client.post("/api/review/pending", json=_BODY).json()
...
def test_push_failure_leaves_no_row(client, monkeypatch):
    def _fail(*a, **k):
    ...
    assert client.post("/api/review/pending", json=_BODY).status_code == 500
