...
**정규식**으로 쓴다. 공백·인용부호·줄바꿈이 바뀌면 거짓 빨강이 난다.
...
def _is_iife_module(src: str) -> bool:
    ...
    통과한다. base 가 1 이 되어 가운데의 진짜 최상위 실행문이 통째로 검사에서 빠진다.
    중간에 깊이가 0 으로 돌아오는 줄이 있는지도 확인한다.
    """
...
def test_viewer_hides_sample_execution_when_the_response_has_no_samples(js):
    """samples 없는 응답(데모의 LeetCode 문제)은 실행 영역을 숨긴다 — 보여도 실행할 예제가 없어 '예제 데이터가 없습니다' 안내만 나온다."""
    src = js["problem-modal.js"]
    body = _js_function_body(src, "async function openProblemModal")
