...
스크립트로 파일을 편집하다 이스케이프가 실제 문자로 치환되는 사고가 반복해서 났다.
그 결과는 조용하다 — 어느 게이트도 빨강이 되지 않고 눈으로도 보이지 않는다.
...
# 확장자가 없지만 편집 대상인 파일들.
_EXTRA_NAMES = {"Dockerfile", ".env.example", ".dockerignore", ".gitattributes"}
...
def test_the_scan_actually_covers_the_repo():
    ...
    assert {"utils.js", "reviews.py", "tokens.css", "index.html", "check_js.sh",
            "Dockerfile", "pyproject.toml", "alembic.ini", ".env.example"} <= names,         f"검사 대상에서 빠진 파일이 있다: {names}"
...
def test_the_deliberate_exceptions_still_exist():
    ...
    for rel, code in _DELIBERATE:
        ...
        assert chr(code) in path.read_text(encoding="utf-8"),             f"{rel} 에 U+{code:04X} 가 더는 없다 — 예외를 지워라"
