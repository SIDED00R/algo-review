"""변이 실험에서 "죽여도 전부 통과" 로 드러난 쿼리들을 고정한다.

전수 리뷰에서 아래를 런타임 치환으로 무력화해도 스위트가 초록이었다.
소비처가 하나뿐이거나 "빈 응답만 단정" 하는 테스트에 가려져 있던 것들이다.
...
def test_average_cf_rating_uses_only_the_latest_submission_per_problem(at_time):
    ...
    예전 테스트는 서로 다른 문제를 1회씩만 저장해, row_number() 중복 제거가 no-op 이었다
    (윈도우 함수를 통째로 지워도 초록이었다). BOJ 쪽은 제대로 고정돼 있어 CF 만 비대칭 누락.
    """
    at_time("2026-01-01T10:00:00")
